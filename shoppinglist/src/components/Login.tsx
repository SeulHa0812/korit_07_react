...
function Login({loginSuccess}: LoginProps) {
  ...
  // Google 로그인 관련
  const handleGoogleLoginSuccess = async (crednetialResponse : CredentialResponse) => {
    console.log('Google Login Success(Frontend상황에서) : ', crednetialResponse );
    if(crednetialResponse.credential) {
      setLoading(true);
      try {
        // Google ID 토큰을 백엔드로 보내서 JWT 받아오는 과정
        const backendJwt = await authenticateWithGoogleToken(crednetialResponse.credential);
  ...
  const handleGoogleLoginError = () => {
    ...
    setErrorMsg('구글 로그인 자체가 실해했습니다. 다시 시도하거나 좀 있다 시도하세요.');
    setOpen(true);
  };
