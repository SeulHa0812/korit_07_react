...
import {AccountCredentials, ShoppingItem, ShoopingItemEntry } from './../../types';
...
export const login = async (creds : AccountCredentials): Promise<string> => {
  try {
    ...
    // error 객체기 AxiosError 인스턴스인지 확인하는 과정 작성
...
export const addItem = async (item: ShoopingItemEntry): Promise<ShoppingItem> => {
  const response = await apiClient.post('/api/items', item, getRequestConfig());
  return response.data;
}

export const updateItem = async (id: number, itemUpdate: ShoopingItemEntry): Promise<ShoppingItem> => {
...
/*
  Google ID 토큰을 백엔드로 보낸 다음 검증하고, 앱의 JWT를 받아옴.
...
export const authenticateWithGoogleToken = async (idToken: string): Promise<string> => {
  ...
  try {
    // 백엔드에 구현한 Google 토큰 검증 엔드포인트를 호출할 것
